        ...
        <AnimatePresence mode="wait">
          {!srtResult ? (
            <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              ...
              {/* Generate Button */}
              <button onClick={handleGenerate} disabled={!file || isProcessing} style={{ width: '100%', padding: '18px', borderRadius: '14px', border: 'none', background: !file || isProcessing ? 'rgba(255,255,255,0.05)' : 'linear-gradient(135deg, #3F59E7, #162DB2)', color: !file || isProcessing ? 'var(--muted)' : '#fff', fontSize: '16px', fontWeight: 700, cursor: !file || isProcessing ? 'not-allowed' : 'pointer', transition: 'all 0.3s ease', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', boxShadow: !file || isProcessing ? 'none' : '0 8px 32px rgba(63,89,231,0.3)' }}>
